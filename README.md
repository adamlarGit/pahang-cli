...
- **Generate Quick Report**: Interactively select 3-tier inspection date folders (`<STATION>/<MONTH>/<DATE>/`) or manual Functional Locations and compile 7-part docx visual reports with dynamic multi-technology template resolution (`DEFECT IR`, `DEFECT IR US`, `DEFECT IR US TEV`), multi-technology defect merging, testsheet equipment spec enrichment, room-based fire safety layouts, and canonical `(IR+US+TEV+VI)` defect suffixes.
- **Substation Post-Processing Pipeline (1-Click)**: 6-stage post-processing pipeline for client deliverables with pre-flight integrity validation across 3-tier Pahang folder hierarchies (`<STATION>/<MONTH>/<DATE>/`), automated renaming synchronization, daily WhatsApp reporting, digital signature stamping / `mode="none"` placeholder sanitization, blank cell diagonal borders, shared COM session automation, per-substation error resilience, and deliverable PDF merging into `QUICK REPORT/<DATE>/<STEM>.pdf`.
