...
- **Automate Raw Material Creation & Sorting**: Validates `TOTAL PE.xlsx` pre-checks, provisions `RAW MATERIAL/` destination folder hierarchies, and matches/copies `IR` and `DG` photos from `UNSORTED RAW DATA/` using testsheet photo range bounds according to the configured camera pattern (`FLIR`, `IR_`/`DC_` dual pairs, `IMG_`, `P` series).
