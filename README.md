...
A unified, interactive Command Line Interface (CLI) for automating Pahang area PE inspection workflows, multi-project workspace management, testsheet parsing, dynamic Quick Report compilation, 1-Click post-processing, and MSMS data pipelines (`v1.16.0`).
...
| **5** | **Generate Quick Report (Visual Report)** | Interactively select 3-tier inspection date folders (`<STATION>/<MONTH>/<DATE>/`) or manual FLs and compile 7-part docx visual reports with dynamic multi-technology template resolution (`DEFECT IR`, `DEFECT IR US`, `DEFECT IR US TEV`), automated pure-Python PRPD phase plot rendering (UE01 FlatBuffers & JSON decoders, 4-tier density scatter bins), testsheet ultrasound/TEV measurement ingestion, Feeder Pillar & Transformer cable type & thermal matrix ingestion, dynamic cell severity shading (`#EE0000` defect, `#00B050` normal), CBM defect detail page rules (VCB heater `ON:<amp>A/OFF:0.0A`, ultrasound default `NORMAL`, busbar position `MAIN`/transition `-`), UltraTEV feeder matching, multi-technology defect merging, and canonical `(IR+US+TEV+VI)` defect suffixes. |
