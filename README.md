...
| **5** | **Generate Quick Report** | Deep module (`generate`/`inspect`) compiling 7-part docx quick reports from inspection date folders (`<STATION>/<MONTH>/<DATE>/`) or manual FLs (single or comma-separated). Features multi-date selection (`Browse Date Folders` interactive checklist with 'a' toggle-all and zero-selection fallback, or `Enter Target Date(s)` with range syntax `..` / `to`), batch multi-date sequential compilation with date-aware progress telemetry, multi-station checkbox pre-selection, batch Word COM session reuse (`WordComDocumentCompiler`), fresh OLE ActiveX container isolation (ADR 0002), table escape paragraphs, retry paste, preflight template fail-fast guards, pure-Python PRPD rendering with non-TEV/US compartment skipping, dynamic multi-technology template resolution (`DEFECT IR`, `DEFECT IR US`, `DEFECT IR US TEV`) with dynamic OpenXML US and TEV cell blanking for switchgear panels, canonical `(IR+US+TEV+VI)` defect suffixes, and pure-domain programmatic CBM defect analysis and recommendation generation across LVDB/FP, Switchgear, and Transformers with dynamic phrasing registries, pure-domain contract scoping (`src/core/contract.py`), and overview table status cell shading. |
...
| **13** | **Generate Full Reports** | Deep module (`generate`/`inspect`) compiling comprehensive 8-part Word full scanning reports (`.docx`) with Executive Summary Census, healthy component scanning pages (`templates/FULL REPORT/NORMAL IR US TEV/`), FLIR Tools+ ActiveX control preservation, automated PRPD graph embedding with non-TEV/US compartment skipping, unified cell shading engine with dynamic switchgear US and TEV OpenXML blanking, dynamic inline defect interleaving under `SubstationIsolatedBatchResiliencePolicy`, automatic multi-part partitioning (`MultiPartPartitionPolicy`) for high-page-volume VCB and GIS switchgear (`Part 01 - Summary`, bay documents, `Part N+2 - TX and Condition`), resilient atomic copy-paste retry handshake, and per-substation Word COM process recycling. Features multi-date selection (`Browse Date Folders` and `Enter Target Date(s)` with range syntax), multi-date date-prefixed substation checklist (`[DD-MM-YYYY] <STEM> [READY]`), date-aware progress telemetry, interactive multi-station checkbox selection with ready pre-selection, dry-run telemetry with multi-part part counts, and batch summary reporting. |
| **14** | **Post-Process Full Reports (PDF + Testsheet Merge)** | Stage 2 post-processing discovering single and multi-part Full Report `.docx` files (`MultiPartPdfStitchingPolicy`), converting finalized documents to PDF via uniform virtual printer (`ComDocumentConverter`), sequentially merging multi-part panel files (`merge_pdfs_batch`), and appending pre-existing testsheet PDFs from `processed_testsheet/pdf/` (`PyPDF2`) into consolidated client deliverables (`FULL REPORT/<DATE>/<STEM>.pdf`). |
