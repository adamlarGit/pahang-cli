...
1. **Two-Method Public Surface**:
   - `QuickReportWorkflow.generate(target: ReportTarget, environment: ProjectEnvironment, *, station: str | None = None, condition_template: Path | None = None, progress_sink: ProgressSink | None = None) -> QuickReportResult`
   - `QuickReportWorkflow.inspect(target: ReportTarget, environment: ProjectEnvironment, *, station: str | None = None, condition_template: Path | None = None) -> QuickReportInspection`
   - `ReportTarget` polymorphically accepts a `Path`, date `str`, sequence of `Path`, or sequence of functional location `str`.
...
6. **Purge of Legacy Shims**:
   ...
   - `QuickReportExtractor.extract()` accepts explicit arguments (`folders`, `fls`, `station`).
