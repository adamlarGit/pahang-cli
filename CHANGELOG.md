...
### Added
- **Full Report Benchmark Auditing & Deliverable Parity (`tests/test_full_report_e2e_benchmarks.py`)**: Audited and validated Full Report generation against canonical benchmarks (`TALAPIA`, `CENDERAWASIH NO.1`, `TELEKOM TANAH PUTIH`), verifying RMU/VCB scanning pages, multi-way FP overviews, interleaved defect pages, PRPD waveforms, condition/sticker pages, and PDF post-processing completeness (closes #38).
- **Core Cell Shading Engine Unification (`src/core/shading.py`)**: Extracted unified cell shading engine as single source of truth across Quick Report and Full Report workflows, standardizing technology severity cell shading (`00B050` green / `EE0000` red), analysis/recommendation banner shading, and eliminating false-positive shading on generic parameter cells.
- **Project-Centric Template Architecture (`src/project/storage.py`, `src/project/environment.py`)**: Added full report template discovery to `WorkspaceStorage` and `ProjectEnvironment`, prioritizing project root templates (`<project_root>/templates/FULL REPORT/`) with graceful repository fallback, and synchronized master templates across active project workspaces.
- **Isolated Per-Substation Batch Workspaces (`src/workflows/full_report.py`)**: Re-architected intermediate file isolation under `.temp/full_report/<SUBSTATION_KEY>/` (`sliced/`, `rendered/`, `prpd/`) with automatic pre-purge on allocation, eliminating batch cross-substation contamination and preventing temporary file leaks into output directories.
- **Automated PRPD Graph Embedding (`src/workflows/full_report.py`, `src/full_report/scan_adapters.py`)**: Wired UltraTEV survey discovery and automated composite PRPD waveform chart generation (Option C default per ADR 0001) for Switchgear panels and Transformer HV sides.
- **Executive Summary Census Table Dynamic Merging (`src/full_report/census.py`)**: Calibrated OpenXML table geometry (11,111 dxa total, 450 dxa row heights), preserved paragraph centering on NO. vertical merges, implemented dynamic vertical merging of Column 1 (`EQUIPMENT`) for transformer groups, formatted switchgear overview as `RMU SF6, TAMCO`, and structured panel rows with per-panel compartment merging.
- **FLIR ActiveX OLE Control Restoration (`templates/FULL REPORT/NORMAL IR US TEV/`)**: Restored `<w:object>` and `<w:control>` containers for FLIR Tools+ ActiveX controls across all 7 normal scanning templates, preserving full interactive thermal tuning capability in compiled `.docx` deliverables.
- **Substation Environment Data Wiring (`src/workflows/full_report.py`, `src/full_report/scan_adapters.py`)**: Wired real testsheet environment values (`time`, `humidity`, `ambient`, `tev_background`) into report context and eliminated invalid `"4"` fallback.
- **Full Report CLI Menu & Workflow Actions (`src/project_workflow_actions.py`, `src/cli_selectors.py`)**: Registered `FullReportAction` ("Generate Full Reports") and `FullReportPostProcessingAction` ("Post-Process Full Reports (PDF + Testsheet Merge)") in `PROJECT_WORKFLOW_ACTIONS` at the bottom of the CLI project workflow menu per D44. Features upfront dry-run telemetry, interactive multi-station checkbox selection with all ready stations pre-checked, confirmation prompt, and execution summary box with total, succeeded, failed, and output paths (closes #37).
- **Full Report Manual FL Input Mode (`src/project_workflow_actions.py`)**: Added top-level selection mode prompt ("Manual FL Input" vs "Select Testsheet Folder") to `generate_full_reports_action()` aligning with `QuickReportAction`, enabling operators to manually specify comma-separated Functional Locations while preserving dry-run telemetry, interactive checkbox selection, and batch execution summary.
- **Substation Name Filtering Resilience (`src/workflows/full_report.py`)**: Enhanced `FullReportWorkflow._matches_station()` to match against `substation_name_erms` and display name in addition to district/area station name, preventing substation filtering drops during multi-station interactive batch runs.
- **Interactive Multi-Station Checkbox Selector (`src/cli_selectors.py`)**: Added `select_substations_interactive()` with automatic `is_ready` pre-selection and status annotations.
...
### Fixed
- **Word COM Teardown Hardening (`src/postprocessing/converters.py`, `src/full_report/slicer.py`, `src/quick_report/compiler.py`)**: Guarded Word COM shutdown with pre-quit `gc.collect()` and explicit proxy clearing to eliminate Windows SEH RPC exceptions (`0x800706be` / `0x800706ba`).
- **Quick Report Defect Detail Banners (`src/quick_report/cbm_render.py`)**: Wired scan post-processing to shade analysis and recommendation banners Red (`EE0000`) on CBM defect pages.
- **Equipment Overview Banner Defect Isolation (`src/full_report/plan_builder.py`, `src/full_report/scan_adapters.py`)**: Isolated defect evaluation per equipment instance so healthy switchgear boards, transformers, and feeder pillars shade Green (`00B050`) while defective instances shade Red (`EE0000`).
