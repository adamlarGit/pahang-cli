...
- [ ] Implement `BrowserPrerequisiteError(RuntimeError)` domain exception:
...
- [ ] Implement `UsTevGraphWorkflow` in `src/workflows/us_tev_graphs.py`:
  - `generate_substation_graphs(survey_root: Path, output_dir: Path, mode: str) -> list[Path]`:
    ...
    - If `mode == "option_b"`:
      ...
      - Overwrites existing image files unconditionally.
- [ ] Implement batch substation processor adhering to `SubstationIsolatedBatchResiliencePolicy`:
...
- [ ] Add unit and workflow tests in `tests/test_us_tev_graph_workflow.py`:
