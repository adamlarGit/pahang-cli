...
- [x] Implement `DiscoveredMeasurement` domain model in `src/quick_report/prpd.py` (or shared discovery module):
...
- [x] Implement canonical label formatter `format_measurement_label(asset, subasset, component, tech)`:
...
- [x] Implement `discover_survey_measurements(survey_root: Path) -> list[DiscoveredMeasurement]`:
...
- [x] Implement deduplication safety:
...
- [x] Add unit test suite in `tests/test_us_tev_discovery.py` using synthetic fixtures:
