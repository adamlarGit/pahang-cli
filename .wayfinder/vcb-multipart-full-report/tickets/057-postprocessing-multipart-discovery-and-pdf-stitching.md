...
- [x] Add `merge_pdfs_batch(self, pdf_paths: Sequence[Path], output_pdf: Path) -> Path` to `DocumentConverter` ABC in `src/postprocessing/converters.py`.
- [x] Implement `merge_pdfs_batch()` in `ComDocumentConverter` using PyPDF2 `PdfWriter` to merge all input PDFs in one pass. Update `FakeDocumentConverter` to record calls in `self.merge_pdfs_batch_calls`. Existing 2-input `merge_pdfs()` remains unchanged.
- [x] Keep `_discover_docx_files()` in `src/workflows/full_report_postprocessing.py` unchanged to preserve its 5-case discovery logic.
- [x] Add private method `_group_multipart_targets(docx_paths: list[Path])` downstream of discovery:
...
- [x] Update `FullReportPostProcessingTelemetry`:
...
- [x] Update `FullReportPostProcessingWorkflow.process()`:
...
- [x] Ensure original Word part documents (`<STEM> - Part *.docx`) remain untouched in the date folder.
- [x] Add unit tests in `tests/test_full_report_postprocessing.py` with `FakeDocumentConverter` testing:
