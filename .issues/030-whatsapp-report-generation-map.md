...
- [Refactor Legacy Workflows to Deep Module + Orchestrator Architecture](file:///.issues/035-refactor-workflows-to-deep-module-architecture.md) — Created ticket to refactor legacy flat workflows (`src/update_qr02_cba_workflow.py` and `src/populate_total_pe_workflow.py`) into `src/master/` deep modules + lean orchestrators for architectural consistency.
